...
function countcategories(){
	require("database/db_connect.php");
	$sql="SELECT * FROM scholarship_categories LIMIT 10";
	if ($result=mysqli_query($con,$sql))
	{
      	//count number of rows in query result
		$rowcount=mysqli_num_rows($result);
		...
		foreach ($result as $categoriescount => $categorydata) {
				#count how many times each category appears in scholarships
			$categoryid=$categorydata['id'];
			$sql="SELECT * FROM scholarships WHERE category='$categoryid'";
			if ($result=mysqli_query($con,$sql)) {
					# code...
...
function getwebname($table){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		if ($rowcount==0) {
      	...
      	//if there are rows available display all the results
		foreach ($result as $titles => $scholarshipname) {
      	# code...
			echo ''.$scholarshipname['website_name'].'';
		}
	}
...
function getcategoriesmenu($table)
{
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		if ($rowcount==0) {
      	...
      	//if there are rows available display all the results
		foreach ($result as $scholarship_categories => $category) {
      	# code...
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $sliderposts => $slideritem) {
      	# code...fetch actual category from categories table
			...
			$sql="SELECT * FROM scholarship_categories WHERE id='$category_id'";
			if ($result=mysqli_query($con,$sql))
			{
			...
			<img src="scholarshipadmin/images/'.$slideritem['photo'].'" alt="fantastic cms" class="img-fluid" style="width:450px;height:350px"/>
...
function getscholarshipridposts($table){
	require("database/db_connect.php");
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		if ($rowcount==0) {
      	...
      	//if there are rows available display all the results
		foreach ($result as $scholarshipprid => $griditem) {
      	# code...
			echo '<div class="col-md-6 scholarship-grid-top">
			<div class="b-grid-top">
			<div class="scholarship_info_left_grid">
			<a href="single.php?id='.$griditem['id'].'">
			<img src="scholarshipadmin/images/'.$griditem['photo'].'" class="img-fluid" alt="fantastic cms" style="width:350px;height:250px">
			</a>
			</div>
			<h3>
...
function getolderposts($table){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $olderposts => $op) {
      	# code...
			echo '<div class="scholarship-grids row mb-3">
			<div class="col-md-5 scholarship-grid-left">
			<a href="single.php?id='.$op['id'].'">
			<img src="scholarshipadmin/images/'.$op['photo'].'" class="img-fluid" alt="fantastic cms">
			</a>
			</div>
			<div class="col-md-7 scholarship-grid-right">
...
function getfour($table){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $thefour => $fourdata) {
      	# code...
			echo '<li>
			<a href="scholarshipadmin/images/'.$fourdata['photo'].'">
			<img src="scholarshipadmin/images/'.$fourdata['photo'].'" alt="fantastic cms" data-desoslide-caption="<h3>Latest Post '.$fourdata['id'].'</h3>">
...
function getonelatest($table){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $onelatest => $onedata) {
      	# code...
			echo '<div class="scholarship-grid-top">
			<div class="b-grid-top">
			<div class="scholarship_info_left_grid">
			<a href="single.php?id='.$onedata['id'].'">
			<img src="scholarshipadmin/images/'.$onedata['photo'].'" class="img-fluid" alt="fantastic cms" style="width:900px;height:500px">
			</a>
			</div>
			<div class="scholarship-info-middle">
			<ul>
			<li>
			<a href="#">
...
function geteditorschoice($table){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $edschoice => $choice) {
			#get actual scholarship post data
			$postid=$choice['scholarship'];
			$sql="SELECT * FROM scholarships WHERE id='$postid'";
			if ($result=mysqli_query($con,$sql)) {
				# code...
					...
					echo '<div class="scholarship-grids row mb-3">
								<div class="col-md-5 scholarship-grid-left">
									<a href="single.php?id='.$postdata['id'].'">
										<img src="scholarshipadmin/images/'.$postdata['photo'].'" class="img-fluid" alt="fantastic cms">
									</a>
								</div>
								<div class="col-md-7 scholarship-grid-right">
...
function getcategoryscholarships($table,$id){
	require("database/db_connect.php");
      	...
      	# code...
			echo '<div class="col-md-6 card">
							<a href="single.php?id='.$cdata['id'].'">
								<img src="scholarshipadmin/images/'.$cdata['photo'].'" class="card-img-top img-fluid" alt="fantastic cms" style="width:480px;height:300px">
							</a>
							<div class="card-body">
								<ul class="scholarship-icons my-4">
									<li>
										<a href="#">
...
function getpopularposts($table){
	...
	if ($result=mysqli_query($con,$sql))
	{
		...
		foreach ($result as $pagehits => $hits) {
      	# code...get actual scholarship from scholarships db
			$storepage=$hits['page'];
			$sql="SELECT * FROM scholarships WHERE title='$storepage'";
			if ($result=mysqli_query($con,$sql)) {
				# code...
				foreach ($result as $allscholarships => $specificscholarship) {
					# code...display the results
					echo '<div class="scholarship-grids row mb-3">
							<div class="col-md-5 scholarship-grid-left">
								<a href="single.php?id='.$specificscholarship['id'].'">
									<img src="scholarshipadmin/images/'.$specificscholarship['photo'].'" class="img-fluid" alt="fantastic cms">
								</a>
							</div>
							<div class="col-md-7 scholarship-grid-right">

								<h5>
									<a href="single.php?id='.$specificscholarship['id'].'">'.$specificscholarship['title'].' </a>
								</h5>
								<div class="sub-meta">
									<span>
										<i class="far fa-clock"></i> '.$specificscholarship['date'].'</span>
								</div>
							</div>
