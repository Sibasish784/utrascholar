...
function editors_choice_insert(){
	...
	$data['scholarship'] = makeSafe($_REQUEST['scholarship']);
		if($data['scholarship'] == empty_lookup_value){ $data['scholarship'] = ''; }
	if($data['scholarship']== ''){
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">" . $Translation['error:'] . " 'scholarship': " . $Translation['field not null'] . '<br><br>';
	...
	sql('insert into `editors_choice` set       `scholarship`=' . (($data['scholarship'] !== '' && $data['scholarship'] !== NULL) ? "'{$data['scholarship']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
...
function editors_choice_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `editors_choice` set       `scholarship`=' . (($data['scholarship'] !== '' && $data['scholarship'] !== NULL) ? "'{$data['scholarship']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function editors_choice_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	$filterer_scholarship = thisOr(undo_magic_quotes($_REQUEST['filterer_scholarship']), '');
	...
	$combo_scholarship = new DataCombo;
	...
	if($selected_id){
		...
		$combo_scholarship->SelectedData = $row['scholarship'];
	}else{
		$combo_scholarship->SelectedData = $filterer_scholarship;
	}
	$combo_scholarship->HTML = '<span id="scholarship-container' . $rnd1 . '"></span><input type="hidden" name="scholarship" id="scholarship' . $rnd1 . '" value="' . html_attr($combo_scholarship->SelectedData) . '">';
	$combo_scholarship->MatchText = '<span id="scholarship-container-readonly' . $rnd1 . '"></span><input type="hidden" name="scholarship" id="scholarship' . $rnd1 . '" value="' . html_attr($combo_scholarship->SelectedData) . '">';
	...
	<script>
		...
		AppGini.current_scholarship__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['scholarship'] : $filterer_scholarship); ?>"};

		jQuery(function() {
			setTimeout(function(){
				if(typeof(scholarship_reload__RAND__) == 'function') scholarship_reload__RAND__();
			}, 10); /* we need to slightly delay client-side execution of the above code to allow AppGini.ajaxCache to work */
		});
		function scholarship_reload__RAND__(){
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>

			$j("#scholarship-container__RAND__").select2({
				/* initial default value */
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_scholarship__RAND__.value, t: 'editors_choice', f: 'scholarship' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
							});
							$j('[name="scholarship"]').val(resp.results[0].id);
							$j('[id=scholarship-container-readonly__RAND__]').html('<span id="scholarship-match-text">' + resp.results[0].text + '</span>');
							if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=scholarships_view_parent]').hide(); }else{ $j('.btn[id=scholarships_view_parent]').show(); }


							if(typeof(scholarship_update_autofills__RAND__) == 'function') scholarship_update_autofills__RAND__();
						}
					});
				},
				width: '100%',
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p: page, t: 'editors_choice', f: 'scholarship' }; },
					results: function(resp, page){ return resp; }
				},
				escapeMarkup: function(str){ return str; }
			}).on('change', function(e){
				AppGini.current_scholarship__RAND__.value = e.added.id;
				AppGini.current_scholarship__RAND__.text = e.added.text;
				$j('[name="scholarship"]').val(e.added.id);
				if(e.added.id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=scholarships_view_parent]').hide(); }else{ $j('.btn[id=scholarships_view_parent]').show(); }


				if(typeof(scholarship_update_autofills__RAND__) == 'function') scholarship_update_autofills__RAND__();
			});

			if(!$j("#scholarship-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_scholarship__RAND__.value, t: 'editors_choice', f: 'scholarship' },
		...
		<?php }else{ ?>
			...
			$j.ajax({
				...
				data: { id: AppGini.current_scholarship__RAND__.value, t: 'editors_choice', f: 'scholarship' },
				success: function(resp){
					$j('[id=scholarship-container__RAND__], [id=scholarship-container-readonly__RAND__]').html('<span id="scholarship-match-text">' + resp.results[0].text + '</span>');
					if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=scholarships_view_parent]').hide(); }else{ $j('.btn[id=scholarships_view_parent]').show(); }
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)){
		$jsReadOnly .= "\tjQuery('#scholarship').prop('disabled', true).css({ color: '#555', backgroundColor: '#fff' });\n";
		$jsReadOnly .= "\tjQuery('#scholarship_caption').prop('disabled', true).css({ color: '#555', backgroundColor: 'white' });\n";
	...
	// process combos
	$templateCode = str_replace('<%%COMBO(scholarship)%%>', $combo_scholarship->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(scholarship)%%>', $combo_scholarship->MatchText, $templateCode);
	$templateCode = str_replace('<%%URLCOMBOTEXT(scholarship)%%>', urlencode($combo_scholarship->MatchText), $templateCode);

	/* lookup fields array: 'lookup field name' => array('parent table name', 'lookup field caption') */
	$lookup_fields = array(  'scholarship' => array('scholarships', 'scholarship'));
	...
	$templateCode = str_replace('<%%UPLOADFILE(scholarship)%%>', '', $templateCode);
	...
	if($selected_id){
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(scholarship)%%>', safe_html($urow['scholarship']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(scholarship)%%>', html_attr($row['scholarship']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(scholarship)%%>', urlencode($urow['scholarship']), $templateCode);
	}else{
		$templateCode = str_replace('<%%VALUE(id)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(id)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(scholarship)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(scholarship)%%>', urlencode(''), $templateCode);
	}
