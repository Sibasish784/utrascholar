	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'scholarship_categories' => array('Blog categories', '', 'resources/table_icons/award_star_bronze_2.png', 'None'),
			'scholarships' => array('Blogs', '', 'resources/table_icons/feed.png', 'None'),
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'scholarship_categories' => "`scholarship_categories`.`id` as 'id', `scholarship_categories`.`name` as 'name'",
			'scholarships' => "`scholarships`.`id` as 'id', `scholarships`.`title` as 'title', IF(    CHAR_LENGTH(`scholarship_categories1`.`name`), CONCAT_WS('',   `scholarship_categories1`.`name`), '') as 'category', `scholarships`.`tags` as 'tags', `scholarships`.`content` as 'content', `scholarships`.`photo` as 'photo', if(`scholarships`.`date`,date_format(`scholarships`.`date`,'%m/%d/%Y'),'') as 'date', `scholarships`.`author` as 'author', `scholarships`.`posted` as 'posted'",
			'editors_choice' => "`editors_choice`.`id` as 'id', IF(    CHAR_LENGTH(`scholarships1`.`title`) || CHAR_LENGTH(`scholarships1`.`author`), CONCAT_WS('',   `scholarships1`.`title`, ' :author ', `scholarships1`.`author`), '') as 'scholarship'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'scholarships' => "`scholarships` LEFT JOIN `scholarship_categories` as scholarship_categories1 ON `scholarship_categories1`.`id`=`scholarships`.`category` ",
			'editors_choice' => "`editors_choice` LEFT JOIN `scholarships` as scholarships1 ON `scholarships1`.`id`=`editors_choice`.`scholarship` "
		);

		$pkey = array(   
			'titles' => 'id',
			'links' => 'id',
			'scholarship_categories' => 'id',
			'scholarships' => 'id',
			'editors_choice' => 'id'
		);
