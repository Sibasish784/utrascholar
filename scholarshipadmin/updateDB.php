<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('scholarship_categories', "create table if not exists `scholarship_categories` (   `id` INT unsigned not null auto_increment , primary key (`id`), `name` VARCHAR(40) not null ) CHARSET utf8", $silent);
		setupTable('scholarships', "create table if not exists `scholarships` (   `id` INT unsigned not null auto_increment , primary key (`id`), `title` VARCHAR(190) not null , `category` INT unsigned not null , `tags` VARCHAR(40) not null , `content` TEXT not null , `photo` VARCHAR(50) not null , `date` DATE , `author` VARCHAR(40) , `posted` VARCHAR(40) not null ) CHARSET utf8", $silent);
		setupIndexes('scholarships', array('category'));
		setupTable('editors_choice', "create table if not exists `editors_choice` (   `id` INT unsigned not null auto_increment , primary key (`id`), `scholarship` INT unsigned not null ) CHARSET utf8", $silent);
		setupIndexes('editors_choice', array('scholarship'));
