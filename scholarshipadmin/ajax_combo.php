...
*/
	...
	$lookups = array(   
		...
		'scholarships' => array(   
			'category' => array(
				'parent_table' => 'scholarship_categories',
				'parent_pk_field' => 'id',
				'parent_caption' => '`scholarship_categories`.`name`',
				'parent_from' => '`scholarship_categories` ',
				'filterers' => array(),
				'custom_query' => '',
		...
		'editors_choice' => array(   
			'scholarship' => array(
				'parent_table' => 'scholarships',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`scholarships`.`title`) || CHAR_LENGTH(`scholarships`.`author`), CONCAT_WS(\'\', `scholarships`.`title`, \' :author \', `scholarships`.`author`), \'\')',
				'parent_from' => '`scholarships` LEFT JOIN `scholarship_categories` as scholarship_categories1 ON `scholarship_categories1`.`id`=`scholarships`.`category` ',
				'filterers' => array(),
				'custom_query' => 'SELECT `scholarships`.`id`, IF(CHAR_LENGTH(`scholarships`.`title`) || CHAR_LENGTH(`scholarships`.`author`), CONCAT_WS(\'\', `scholarships`.`title`, \' :author \', `scholarships`.`author`), \'\') FROM `scholarships` LEFT JOIN `scholarship_categories` as scholarship_categories1 ON `scholarship_categories1`.`id`=`scholarships`.`category` WHERE posted=\'publish\' ORDER BY 2',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => true
			)
